...
let selectedNotesDate = null;    // dateKey currently open in the day-notes board
let logoTapCount = 0;
...
function handleLogout() {
  ...
  currentMember = null;
  myRecords = [];
  selectedNotesDate = null;
  document.getElementById("day-notes-board").hidden = true;
...
function renderCalendar() {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (key === selectedNotesDate) cell.classList.add("is-selected");

    // Days that have already "initialized" (today or earlier) can be tapped
    // to open the notes board; future days have no data yet, so they stay
    // inert.
    if (!isFuture) {
      cell.classList.add("is-clickable");
      cell.tabIndex = 0;
      cell.setAttribute("role", "button");
      cell.setAttribute("aria-label", `View notes for ${key}`);
      cell.addEventListener("click", () => openDayNotesBoard(key));
      cell.addEventListener("keydown", (e) => {
        if (e.key === "Enter" || e.key === " ") {
          e.preventDefault();
          openDayNotesBoard(key);
        }
      });
    }
...
/* ---------------------------------------------------------
   8b. DAY NOTES BOARD — shows what every member wrote on a
   given (already-initialized) day, once tapped from the
   calendar grid above.
   --------------------------------------------------------- */

async function openDayNotesBoard(key) {
  selectedNotesDate = key;
  renderCalendar(); // re-render just to apply the .is-selected highlight

  const board = document.getElementById("day-notes-board");
  const title = document.getElementById("day-notes-title");
  const list = document.getElementById("day-notes-list");

  const niceDate = new Date(key + "T00:00:00").toLocaleDateString(undefined, {
    weekday: "long",
    month: "long",
    day: "numeric",
    year: "numeric"
  });
  title.textContent = `Notes · ${niceDate}`;
  list.innerHTML = `<p class="muted-text">Loading notes…</p>`;
  board.hidden = false;
  board.scrollIntoView({ behavior: "smooth", block: "nearest" });

  try {
    const [{ data: records, error: rErr }, { data: members, error: mErr }] = await Promise.all([
      supabaseClient
        .from("daily_tracking")
        .select("member_id, bible_reading, prayer, scout_activity, notes")
        .eq("date", key),
    ...
    const nameLookup = new Map((members || []).map((m) => [m.id, m.name]));

    const entries = (records || [])
      .map((r) => ({
        name: nameLookup.get(r.member_id) || "Unknown",
        bible_reading: !!r.bible_reading,
        prayer: !!r.prayer,
        scout_activity: !!r.scout_activity,
        notes: (r.notes || "").trim()
      }))
      .sort((a, b) => a.name.localeCompare(b.name));

    if (!entries.length) {
      list.innerHTML = `<p class="muted-text">No one had checked in on this day.</p>`;
      return;
    }
      ...
      .map(
        ...
        <div class="day-note-card">
          <div class="day-note-head">
            <span class="lb-avatar">${escapeHtml(getInitials(e.name))}</span>
            <span class="day-note-name">${escapeHtml(e.name)}</span>
            <span class="day-note-tags">
              ${e.bible_reading ? '<span class="tag tag-done">Bible</span>' : ""}
              ${e.prayer ? '<span class="tag tag-done">Prayer</span>' : ""}
              ${e.scout_activity ? '<span class="tag tag-done">Activity</span>' : ""}
            </span>
          </div>
          <p class="day-note-text">${
            e.notes ? escapeHtml(e.notes) : '<span class="muted-text">No notes written.</span>'
          }</p>
        </div>`
      )
      .join("");
    ...
    console.error(err);
    list.innerHTML = `<p class="muted-text">Could not load notes for this day.</p>`;
  }
}

function closeDayNotesBoard() {
  selectedNotesDate = null;
  document.getElementById("day-notes-board").hidden = true;
  renderCalendar();
}
...
function wireEvents() {
  ...
  document.getElementById("day-notes-close").addEventListener("click", closeDayNotesBoard);
