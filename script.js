...
function renderTodayCard() {
  document.querySelectorAll(".checkin-card").forEach((card) => {
    ...
    card.querySelector(".checkin-state").textContent = done ? "Completed today ✓" : "Tap to mark done";
  });
  document.getElementById("today-notes").value = todaysDraft.notes || "";
}
...
async function saveTodaysCheckin() {
  ...
  try {
    ...
    feedback.textContent = "Saved! Keep up the streak 🔥";
...
async function loadLeaderboard() {
  ...
  try {
    const [{ data: members, error: mErr }, { data: records, error: rErr }] = await Promise.all([
      ...
      supabase.from("daily_tracking").select("member_id, date, bible_reading, prayer, scout_activity")
    ]);
    if (mErr) throw mErr;
    if (rErr) throw rErr;
